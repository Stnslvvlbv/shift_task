    ...
    def _apply_sorting(query, sort_by: Optional[str], sort_order: str, model_orm: Base):
        ...
        if sort_by:
            ...
            if hasattr(model_orm, sort_by):
                ...
                raise ValueError(
                    f"Предупреждение: Поле '{sort_by}' не найдено в модели."
                )
        return query

    async def _apply_pagination_and_count(
        self, query, limit: int = None, offset: int = None
    ) -> Tuple:
        """
        ...
        try:
            ...
            limit = (
                limit if limit is not None else 15
            )  # По умолчанию 15 записей на страницу
            offset = offset if offset is not None else 0  # По умолчанию смещение 0

            # Проверяем корректность параметров offset и limit
            if offset < 0 or limit <= 0:
                raise HTTPException(
                    status_code=400, detail="Некорректные параметры offset или limit"
                )
            ...
            # Применяем пагинацию
            effective_limit = (
                min(limit, total_count - offset) if total_count > offset else 0
            )
